...
function isTriangle(a, b, c) {
  return a + b > c && a + c > b && b + c > a; // а как тут цикл то использовать? ¯\_(ツ)_/¯
}
 ...
 */
function getDigitalRoot(num) { // WTF????
  let coolNum = num;
  while (coolNum > 9) {
    coolNum = String(coolNum).split('').map(Number).reduce((acc, digit) => acc + digit, 0);
  }
  return coolNum;
}
