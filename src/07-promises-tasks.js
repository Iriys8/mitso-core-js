...
function getFastestPromise(array) {
  return Promise.race(array); // буквально race
}
...
function chainPromises(promises, action) {
  ...
  const resultsPromise = promises.reduce((accumulator, promise) => accumulator.then(
    (results) => promise.then(
      (result) => [...results, result],
    ).catch(() => results),
  ), initialValue);
  return resultsPromise.then((results) => results.reduce(action));
}
